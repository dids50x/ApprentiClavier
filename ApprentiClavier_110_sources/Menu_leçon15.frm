VERSION 4.00
Begin VB.Form Menu_leçon15 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Menu leçon 15"
   ClientHeight    =   5535
   ClientLeft      =   600
   ClientTop       =   1935
   ClientWidth     =   9810
   ...
   Begin VB.ListBox List1 
      ...
      EndProperty
      Height          =   1785
      ItemData        =   "Menu_leçon15.frx":0000
      Left            =   120
      List            =   "Menu_leçon15.frx":000D
      TabIndex        =   0
      Top             =   960
      Width           =   8085
   End
   ...
   End
End
Attribute VB_Name = "Menu_leçon15"
...
Menu_leçon15.Quitter.Caption = msgQuitterMP & msgÉchap

'Paramètres
If numleçon <> 17 Then numexo = 0
numleçon = 17   ' Toujours leçon n + 2
Set menu_courant = Menu_leçon15
Set menu_suivant = Menu_leçon16
Module_routines.Colors Menu_leçon15  '12/2011
Module_routines.Dimension Menu_leçon15
Module_menus.menu_reset "menu_leçon15.txt"
Module_routines.menu_refresh "menu_courant.txt", Menu_leçon15
Module_routines.mshow Menu_leçon15
Label3.Caption = bannerVersion & ", " & bannerCopyright
Module_routines.niveaux
Module_routines.MenuEditorTrans Menu_leçon15
...
Menu_leçon15.Caption = debexplivalue & bannerMenu & debgenvalue & bannerLeçon & " 15"
Menu_leçon15.label1.Caption = msgChoisissez
...
If list1.ListIndex = 0 Then
    numexo = 0
    Unload Menu_leçon15
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon15A.txt")
    If tempo = "" Then
ML10:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon15A.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML11:
    ...
    If msgf = 1 Then
        Module_routines.placeinmsgtext0 "leçon15A.txt"
        vmsgbox = MsgBox(msgtext0, 1, msgDictée & "15 A.")
        If vmsgbox = 2 Then
            keyinhibit = 1
            Menu_leçon15.Show 1
            Exit Sub
        End If
        exo_courant = "leçon15A.txt"
            
        ' Msg d'encouragements et d'explications
        Module_routines.resetmsg
         
        ' Go
        leçon14.Caption = bannerLeçon & " 15 A"  'Pas de point dans le titre, car sono avec ponctuation
        leçon14.Show 1
    End If
    If msgf = 0 Then Menu_leçon15.Show 1
End If
...
If list1.ListIndex = 1 Then
    numexo = 1
    Unload Menu_leçon15
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon15B.txt")
    If tempo = "" Then
ML20:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon15B.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML21:
    ...
    If msgf = 1 Then
        Module_routines.placeinmsgtext0 "leçon15B.txt"
        vmsgbox = MsgBox(msgtext0, 1, msgDictée & "15 B.")
        If vmsgbox = 2 Then
            keyinhibit = 1
            Menu_leçon15.Show 1
            Exit Sub
        End If
        exo_courant = "leçon15B.txt"
                        
        ' Msg d'encouragements et d'explications
        Module_routines.resetmsg
            
        ' Go
        leçon14.Caption = bannerLeçon & " 15 B"  'Pas de point dans le titre, car sono avec ponctuation
        leçon14.Show 1
    End If
    If msgf = 0 Then Menu_leçon15.Show 1
End If
...
If list1.ListIndex = 2 Then
    numexo = 2
    Unload Menu_leçon15
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon15C.txt")
    If tempo = "" Then
ML30:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon15C.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML31:
    ...
    If msgf = 1 Then
        Module_routines.placeinmsgtext0 "leçon15C.txt"
        vmsgbox = MsgBox(msgtext0, 1, msgDictée & "15 C.")
        If vmsgbox = 2 Then
            keyinhibit = 1
            Menu_leçon15.Show 1
            Exit Sub
        End If
        exo_courant = "leçon15C.txt"
                        
        ' Msg d'encouragements et d'explications
        Module_routines.resetmsg
            
        ' Go
        leçon14.Caption = bannerLeçon & " 15 C"  'Pas de point dans le titre, car sono avec ponctuation
        leçon14.Show 1
    End If
    If msgf = 0 Then Menu_leçon15.Show 1
End If
...
If Keycode = 91 Or Keycode = 92 Then Module_routines.cancelwin 0, Menu_leçon15, 1
End Sub
