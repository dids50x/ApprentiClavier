VERSION 4.00
Begin VB.Form Menu_leçon3 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Menu leçon 3"
   ClientHeight    =   6015
   ClientLeft      =   600
   ClientTop       =   1935
   ClientWidth     =   9855
   ...
   Begin VB.ListBox List1 
      ...
      EndProperty
      Height          =   1785
      ItemData        =   "Menu_leçon3.frx":0000
      Left            =   120
      List            =   "Menu_leçon3.frx":000D
      TabIndex        =   0
      Top             =   960
      Width           =   8445
   End
...
Menu_leçon3.Quitter.Caption = msgQuitterMP & msgÉchap

'Paramètres
If numleçon <> 5 Then numexo = 0
numleçon = 5   ' Toujours leçon n + 2
Set menu_courant = Menu_leçon3
Set menu_suivant = Menu_leçon4
Module_routines.Colors Menu_leçon3  '12/2011
Module_routines.Dimension Menu_leçon3
Module_menus.menu_reset "menu_leçon3.txt"
Module_routines.menu_refresh "menu_courant.txt", Menu_leçon3
Module_routines.mshow Menu_leçon3
Label3.Caption = bannerVersion & ", " & bannerCopyright
Module_routines.niveaux
Module_routines.MenuEditorTrans Menu_leçon3
...
Menu_leçon3.Caption = debexplivalue & bannerMenu & debgenvalue & bannerLeçon & " 3"
Menu_leçon3.label1.Caption = msgChoisissez
...
'****************  EXERCICE 3A ********************************************
' MENU : La rangée  W X C V
If list1.ListIndex = 0 Then
    numexo = 0  ' 0 pour A
    Unload Menu_leçon3
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon3A.txt")
    If tempo = "" Then
ML10:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon3A.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML14:
                ...
                If msgf = 1 Then
                    ...
                    ' Go
                    Leçon2et3.Caption = bannerLeçon & " 3 A."
                    Leçon2et3.Show 1
                End If
            End If
        End If
    End If
    If msgf = 0 Then Menu_leçon3.Show 1
End If

'****************  EXERCICE 3B ********************************************
' MENU : Les consonnes difficiles à écouter
If list1.ListIndex = 1 Then
    numexo = 1
    Unload Menu_leçon3
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon3B.txt")
    If tempo = "" Then
ML20:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon3B.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML22:
        ...
        If msgf = 1 Then
            exo_courant = "leçon3B.txt"
            
            ' Msg d'encouragements et d'explications
            Module_routines.resetmsg
            
            ' Go
            Leçon2et3.Caption = bannerLeçon & " 3 B."
            Leçon2et3.Show 1
        End If
    End If
    If msgf = 0 Then Menu_leçon3.Show 1
...
'****************  EXERCICE 3C ********************************************
' MENU : Les mots ambigus à écouter
If list1.ListIndex = 2 Then
    numexo = 2
    Unload Menu_leçon3
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon3C.txt")
    If tempo = "" Then
ML30:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon3C.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML31:
    ...
    If msgf = 1 Then
        exo_courant = "leçon3C.txt"
        
        ' Msg d'encouragements et d'explications
        Module_routines.resetmsg
            
        ' Go
        Leçon2et3bis.Caption = bannerLeçon & " 3 C."
        Leçon2et3bis.Show 1
    End If
    If msgf = 0 Then Menu_leçon3.Show 1
End If
...
If Keycode = 91 Or Keycode = 92 Then Module_routines.cancelwin 0, Menu_leçon3, 1
End Sub
