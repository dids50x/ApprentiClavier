VERSION 4.00
Begin VB.Form Menu_leçon17 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Menu leçon 17"
   ClientHeight    =   5640
   ClientLeft      =   600
   ClientTop       =   1890
   ClientWidth     =   9855
   ...
   Begin VB.ListBox List1 
      ...
      EndProperty
      Height          =   2130
      ItemData        =   "Menu_leçon17.frx":0000
      Left            =   120
      List            =   "Menu_leçon17.frx":0002
      TabIndex        =   0
      Top             =   720
      Width           =   9375
   End
   ...
   End
End
Attribute VB_Name = "Menu_leçon17"
Attribute VB_Creatable = False
Attribute VB_Exposed = False
'*******************************  LOAD  ************************************************
' MENU : Toutes les touches, avec dextérité
Private Sub Form_Load()
If FullScreenSwitch = 1 Then WindowState = 2
Menu_leçon17.Quitter.Caption = msgQuitterMP & msgÉchap

'Paramètres
If numleçon <> 19 Then numexo = 0
numleçon = 19   ' Toujours leçon n + 2
Set menu_courant = Menu_leçon17
Set menu_suivant = Menu_leçon18
Module_routines.Colors Menu_leçon17  '12/2011
Module_routines.Dimension Menu_leçon17
Module_menus.menu_reset "menu_leçon17.txt"
Module_routines.menu_refresh "menu_courant.txt", Menu_leçon17
Module_routines.mshow Menu_leçon17
Label3.Caption = bannerVersion & ", " & bannerCopyright
Module_routines.niveaux
Module_routines.MenuEditorTrans Menu_leçon17
...
Menu_leçon17.Caption = debexplivalue & bannerMenu & debgenvalue & bannerLeçon & " 17"
Menu_leçon17.label1.Caption = msgChoisissez
...
If list1.ListIndex = 0 Then
    numexo = 0  ' 0 pour A
    Unload Menu_leçon17
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon17A.txt")
    If tempo = "" Then
ML10:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon17A.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML11:
    ...
    If msgf = 1 Then
        ...
        Leçon5.Caption = bannerLeçon & " 17 A."
        Leçon5.Show 1
    End If
    If msgf = 0 Then Menu_leçon17.Show 1
End If
...
If list1.ListIndex = 1 Then
    numexo = 1
    Unload Menu_leçon17
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon17B.txt")
    If tempo = "" Then
ML20:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon17B.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML21:
    ...
    If msgf = 1 Then
        ...
        leçon13.Caption = bannerLeçon & " 17 B."
        leçon13.Show 1
    End If
    If msgf = 0 Then Menu_leçon17.Show 1
End If
...
'****************  EXERCICE 17C ********************************************
' MENU : Vitesse sur tous les caractères et combinaisons
If list1.ListIndex = 2 Then
    numexo = 2
    Unload Menu_leçon17
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon17C.txt")
    If tempo = "" Then
ML30:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon17C.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML31:
    ...
    If msgf = 1 Then
        exo_courant = "leçon17C.txt"
            
        ' Msg d'encouragements et d'explications
        Module_routines.resetmsg
            
        ' Go
        timevalid = 2 ' autorise l'affichage et le calcul de la vitesse en mode cumulé
        notab = 0  ' autorise la touche TAB
        leçon13.Caption = bannerLeçon & " 17 C."
        leçon13.Show 1
    End If
    If msgf = 0 Then Menu_leçon17.Show 1
End If
...
'****************  EXERCICE 17D ********************************************
' MENU : Vitesse sur le pavé numérique
If list1.ListIndex = 3 Then
    numexo = 3
    Unload Menu_leçon17
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon17D.txt")
    If tempo = "" Then
ML40:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon17D.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML41:
    ...
    If msgf = 1 Then
        exo_courant = "leçon17D.txt"
            
        ' Msg d'encouragements et d'explications
        Module_routines.resetmsg
            
        ' Go
        numpad = 1  'Mode numérique par défaut pour le pavé numérique
        timevalid = 2 ' autorise l'affichage et le calcul de la vitesse en mode cumulé
        notab = 0  ' autorise la touche TAB
        leçon13.Label4.Left = 0.7 * leçon13.Width '12/2011 ajouts
        leçon13.text5.Left = 0.83 * leçon13.Width
        leçon13.text5.Width = 0.1 * leçon13.Width
        leçon13.Caption = bannerLeçon & " 17 D."
        leçon13.Show 1
    End If
    If msgf = 0 Then Menu_leçon17.Show 1
End If
...
If Keycode = 91 Or Keycode = 92 Then Module_routines.cancelwin 0, Menu_leçon17, 1
End Sub
