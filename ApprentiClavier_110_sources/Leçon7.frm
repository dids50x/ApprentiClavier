...
noalt = 0
iwait = 0
Set leçon_courante = leçon7
Module_routines.Colors leçon7  '12/2011
Module_routines.Dimension leçon7
...
Module_routines.mshow leçon7 'avril 2008
...
' Première currentline (après le bip 2 tons, sinon bip inaudible en Win 98)
...
Else
    ...
    leçonfontsize = (20 * leçon7.Width) / 11746  ' don't exagerate text size, it would be out screen
    Module_routines.AdjustWidthAndSize leçon7, 1
End If
End Sub
...
leçon7.text1.Text = currentline
leçon7.text1.SelStart = 0
leçon7.text1.SelLength = Len(leçon7.text1.Text)
nbcaras = Len(leçon7.text1.Text)
...
Module_routines.text2text1 indif, sonocara, 0, 1, 0, 0
...
' Attendre que le timer1 ait agi pour valider l'affichage
If iwait = 1 Then
    scorecourant = CInt(pctt) & " %.   " & nbmots & msgMots & elapsedtot + elapsed & msgSecondes
    leçon7.text5.Text = scorecourant
End If
End Sub
...
If Keycode = 91 Or Keycode = 92 Then
    Module_routines.cancelwin 0, leçon7, 0
    Exit Sub
End If
...
'RetArr interdit
If Keycode = 8 Then
    Module_routines.bip leçon7
    t2inhibit = 1
    ii = ii + 1
    leçon7.text2.Text = Left(leçon7.text1.Text, ii)
    t2inhibit = 0
    leçon7.text2.SelStart = Len(leçon7.text2.Text)
End If
...
If Keycode = 27 Then
    ...
    If keyinhibit = 2 Then
        keyinhibit = 0
        leçon7.text4.Visible = False
        leçon7.text1.SelLength = 0
...
'Touche F1
If Keycode = 112 Then Module_routines.help_f1 leçon7
...
'Touche F3
If Keycode = 114 Then Module_routines.help_f3 leçon7
