VERSION 4.00
Begin VB.Form Menu_leçon5 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Menu leçon 5"
   ClientHeight    =   5790
   ClientLeft      =   600
   ClientTop       =   1860
   ClientWidth     =   9840
   ...
   Begin VB.ListBox List1 
      ...
      EndProperty
      Height          =   1785
      ItemData        =   "Menu_leçon5.frx":0000
      Left            =   120
      List            =   "Menu_leçon5.frx":000D
      TabIndex        =   0
      Top             =   840
      Width           =   8880
   End
   ...
   End
End
Attribute VB_Name = "Menu_leçon5"
Attribute VB_Creatable = False
Attribute VB_Exposed = False
'*****************  LOAD  ************************************************
' MENU : Mots, proverbes, phrases
Private Sub Form_Load()
If FullScreenSwitch = 1 Then WindowState = 2
Menu_leçon5.Quitter.Caption = msgQuitterMP & msgÉchap

'Paramètres
If numleçon <> 7 Then numexo = 0
numleçon = 7   ' Toujours leçon n + 2
Set menu_courant = Menu_leçon5
Set menu_suivant = Menu_leçon6
Module_routines.Colors Menu_leçon5  '12/2011
Module_routines.Dimension Menu_leçon5
Module_menus.menu_reset "menu_leçon5.txt"
Module_routines.menu_refresh "menu_courant.txt", Menu_leçon5
Module_routines.mshow Menu_leçon5
Label3.Caption = bannerVersion & ", " & bannerCopyright
Module_routines.niveaux
Module_routines.MenuEditorTrans Menu_leçon5
...
Menu_leçon5.Caption = debexplivalue & bannerMenu & debgenvalue & bannerLeçon & " 5"
Menu_leçon5.label1.Caption = msgChoisissez
...
If list1.ListIndex = 0 Then
    numexo = 0
    Unload Menu_leçon5
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon5A.txt")
    If tempo = "" Then
ML10:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon5A.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML12:
        ...
        If msgf = 1 Then
            ...
            Leçon5.Caption = bannerLeçon & " 5 A."
            Leçon5.Show 1
        End If
    End If
    If msgf = 0 Then Menu_leçon5.Show 1
End If
...
If list1.ListIndex = 1 Then
    numexo = 1
    Unload Menu_leçon5
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon5B.txt")
    If tempo = "" Then
ML20:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon5B.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML22:
        pagenum = 2: pagemax = 1
        msgtext0 = CRLF + pg5b2 + pressez
        Msgform.Show 1
        If msgf = 33 Then GoTo ML21
        If msgf = 34 Then Beep
        If msgf = 2 Or msgf = 34 Then GoTo ML22
        If msgf = 1 Then
            exo_courant = "leçon5B.txt"
                        
            ' Msg d'encouragements et d'explications
            Module_routines.resetmsg
            
            ' Go
            Leçon5.Caption = bannerLeçon & " 5 B."
            Leçon5.Show 1
        End If
    End If
    If msgf = 0 Then Menu_leçon5.Show 1
...
If list1.ListIndex = 2 Then
    numexo = 2
    Unload Menu_leçon5
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon5C.txt")
    If tempo = "" Then
ML30:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon5C.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML31:
    pagenum = 0
    msgtext0 = CRLF + pg5c0 + pressez
    Msgform.Show 1
    If msgf = 2 Then GoTo ML31
    If msgf = 1 Then
        exo_courant = "leçon5C.txt"
            
        ' Msg d'encouragements et d'explications
        Module_routines.resetmsg
        
        ' Go
        Leçon5.Caption = bannerLeçon & " 5 C."
        Leçon5.Show 1
    End If
    If msgf = 0 Then Menu_leçon5.Show 1
End If
...
If Keycode = 91 Or Keycode = 92 Then Module_routines.cancelwin 0, Menu_leçon5, 1
End Sub
