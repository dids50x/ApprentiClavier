VERSION 4.00
Begin VB.Form Menu_leçon11 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Menu leçon 11"
   ClientHeight    =   5805
   ClientLeft      =   600
   ClientTop       =   1980
   ClientWidth     =   9885
   ...
   Begin VB.ListBox List1 
      ...
      EndProperty
      Height          =   1785
      ItemData        =   "Menu_leçon11.frx":0000
      Left            =   120
      List            =   "Menu_leçon11.frx":0002
      TabIndex        =   0
      Top             =   840
      Width           =   9735
   End
   ...
   End
End
Attribute VB_Name = "Menu_leçon11"
Attribute VB_Creatable = False
Attribute VB_Exposed = False
'*****************  LOAD  ************************************************
' MENU : La vitesse : le trop et le galop
Private Sub Form_Load()
If FullScreenSwitch = 1 Then WindowState = 2
Menu_leçon11.Quitter.Caption = msgQuitterMP & msgÉchap

'Paramètres
If numleçon <> 13 Then numexo = 0
numleçon = 13   ' Toujours leçon n + 2
Set menu_courant = Menu_leçon11
Set menu_suivant = Menu_leçon12
Module_routines.Colors Menu_leçon11  '12/2011
Module_routines.Dimension Menu_leçon11
Module_menus.menu_reset "menu_leçon11.txt"
Module_routines.menu_refresh "menu_courant.txt", Menu_leçon11
Module_routines.mshow Menu_leçon11
Label3.Caption = bannerVersion & ", " & bannerCopyright
Module_routines.niveaux
Module_routines.MenuEditorTrans Menu_leçon11
...
Menu_leçon11.Caption = debexplivalue & bannerMenu & debgenvalue & bannerLeçon & " 11"
Menu_leçon11.label1.Caption = msgChoisissez
...
If list1.ListIndex = 0 Then
    numexo = 0
    Unload Menu_leçon11
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon11A.txt")
    If tempo = "" Then
ML10:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon11A.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML11:
    pagenum = 0
    msgtext0 = CRLF + pg11a0 + pressez
    Msgform.Show 1
    If msgf = 2 Then GoTo ML11
    If msgf = 1 Then
        exo_courant = "leçon11A.txt"
        
        ' Msg d'encouragements et d'explications
        Module_routines.resetmsg
        
        ' Go
        leçon7.Caption = bannerLeçon & " 11 A."
        leçon7.Show 1
    End If
    If msgf = 0 Then Menu_leçon11.Show 1
End If
...
If list1.ListIndex = 1 Then
    numexo = 1
    Unload Menu_leçon11
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon11B.txt")
    If tempo = "" Then
ML20:
    pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon11B.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML22:
        ...
        If msgf = 1 Then
            ...
            ' Go
            iwrongbismax = 2
            leçon7.Caption = bannerLeçon & " 11 B."
            leçon7.Show 1
        End If
    End If
    If msgf = 0 Then Menu_leçon11.Show 1
...
If list1.ListIndex = 2 Then
    numexo = 2
    Unload Menu_leçon11
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon11C.txt")
    If tempo = "" Then
ML30:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon11C.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML31:
    pagenum = 0
    msgtext0 = CRLF + pg11c0 + pressez
    Msgform.Show 1
    If msgf = 2 Then GoTo ML31
    If msgf = 1 Then
        exo_courant = "leçon11C.txt"
        ...
        ' Go
        iwrongbismax = 2
        leçon7.Caption = bannerLeçon & " 11 C."
        leçon7.Show 1
    End If
    If msgf = 0 Then Menu_leçon11.Show 1
End If
...
If Keycode = 91 Or Keycode = 92 Then Module_routines.cancelwin 0, Menu_leçon11, 1
End Sub
