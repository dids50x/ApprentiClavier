VERSION 4.00
Begin VB.Form Menu_leçon7 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Menu leçon 7"
   ClientHeight    =   5520
   ClientLeft      =   600
   ClientTop       =   1920
   ClientWidth     =   9930
   ...
   Begin VB.ListBox List1 
      ...
      EndProperty
      Height          =   1785
      ItemData        =   "Menu_leçon7.frx":0000
      Left            =   120
      List            =   "Menu_leçon7.frx":0002
      TabIndex        =   0
      Top             =   720
      Width           =   9255
   End
...
Menu_leçon7.Quitter.Caption = msgQuitterMP & msgÉchap

'Paramètres
If numleçon <> 9 Then numexo = 0
numleçon = 9   ' Toujours leçon n + 2
Set menu_courant = Menu_leçon7
Set menu_suivant = Menu_leçon8
Module_routines.Colors Menu_leçon7  '12/2011
Module_routines.Dimension Menu_leçon7
Module_menus.menu_reset "menu_leçon7.txt"
Module_routines.menu_refresh "menu_courant.txt", Menu_leçon7
Module_routines.mshow Menu_leçon7
Label3.Caption = bannerVersion & ", " & bannerCopyright
Module_routines.niveaux
Module_routines.MenuEditorTrans Menu_leçon7
...
Menu_leçon7.Caption = debexplivalue & bannerMenu & debgenvalue & bannerLeçon & " 7"
Menu_leçon7.label1.Caption = msgChoisissez
...
If list1.ListIndex = 0 Then
    numexo = 0
    Unload Menu_leçon7
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon7A.txt")
    If tempo = "" Then
ML10:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon7A.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML12:
        ...
        If msgf = 1 Then
            exo_courant = "leçon7A.txt"
        
            ' Msg d'encouragements et d'explications
            Module_routines.resetmsg
        
            ' Go
            leçon7.Caption = bannerLeçon & " 7 A."
            leçon7.Show 1
        End If
    End If
    If msgf = 0 Then Menu_leçon7.Show 1
End If
...
If list1.ListIndex = 1 Then
    numexo = 1
    Unload Menu_leçon7
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon7B.txt")
    If tempo = "" Then
ML20:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon7B.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML22:
        ...
        If msgf = 1 Then
            ...
            ' Go
            iwrongbismax = 2
            leçon7.Caption = bannerLeçon & " 7 B."
            leçon7.Show 1
        End If
    End If
    If msgf = 0 Then Menu_leçon7.Show 1
...
If list1.ListIndex = 2 Then
    numexo = 2
    Unload Menu_leçon7
    tempo = Dir(vpath & "Leçons\" & nivoRep & "\leçon7C.txt")
    If tempo = "" Then
ML30:
        pagenum = 0
        msgtext0 = CRLF + msgNofic & vpath & "Leçons\" & nivoRep & "\leçon7C.txt. " + perso_methode + pressez
        Msgform.Show 1
...
ML31:
    ...
    If msgf = 1 Then
        ...
        ' Go
        iwrongbismax = 2
        leçon7.Caption = bannerLeçon & " 7 C."
        leçon7.Show 1
    End If
    If msgf = 0 Then Menu_leçon7.Show 1
End If
...
If Keycode = 91 Or Keycode = 92 Then Module_routines.cancelwin 0, Menu_leçon7, 1
End Sub
